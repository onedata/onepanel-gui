...
async function testNotAllowPathTypeEdit(storageData, storageType = storageData.type) {
  it(`does not allow to edit path type of storage with type "${storageType}"`, async function () {
    ...
    await render(hbs `<ClusterStorageAddForm2
      @storage={{storage}}
      @mode={{mode}}
...
async function testAllowCertainPathTypeCreate({
  ...
  it(`${allowText} to set "${pathType}" path type of storage with type "${type}"`, async function () {
    ...
    await render(hbs`<ClusterStorageAddForm2 @mode={{mode}} />`);
...
async function checkForStorageDetailsInShowMode(type, storage, fieldCount) {
  it(`shows storage details for ${type} type`, async function () {
    ...
    await render(hbs `<ClusterStorageAddForm2 @storage={{storage}} @mode="show" />`);
...
async function checkForStorageDetailsInEditMode(type, storage, fieldCount) {
  it(`shows storage details for ${type} type`, async function () {
    this.set('storage', storage);
    await render(hbs`<ClusterStorageAddForm2 @storage={{storage}} @mode="edit" />`);
    expect(findAll('.form-group:has(>label)')).to.have.length(fieldCount);
    Object.entries(storage).forEach(([key, value]) => {
...
async function checkForStorageDetailsInCreateMode(type, fieldCount) {
  it(`shows storage details for ${type} type`, async function () {
    await render(hbs`<ClusterStorageAddForm2 @mode="create" />`);
    await selectChoose('.type-field .dropdown-field', type);
    expect(findAll('.form-group:has(>label)')).to.have.length(fieldCount);
  });
}

describe('Integration | Component | cluster-storage-add-form2', function () {
  setupRenderingTest();
  ...
  context('in create mode', function () {
    ...
    it('resets fields values after form visibility toggle', async function () {
      ...
      await render(hbs`<ClusterStorageAddForm2 @isFormOpened={{isFormOpened}} />`);
    ...
    it(
      ...
      async function () {
        await render(hbs `
          <ClusterStorageAddForm2 @mode="create" />
        `);
        await selectChoose('.type-field .dropdown-field', POSIX_TYPE.name);
  ...
  context('in edit mode', function () {
    ...
    it('luma enabled toggle does not change luma fields values', async function () {
      ...
      await render(hbs `
        <ClusterStorageAddForm2 @storage={{storage}} @mode="edit" />
      `);
    ...
    it('submits null value for cleared out optional fields', async function () {
      ...
      this.set('storage', POSIX_STORAGE);
      await render(hbs `
        <ClusterStorageAddForm2 @storage={{storage}} @mode="edit" @submit={{submit}} />
      `);
    ...
    it('does not remember editor values while edit -> show -> edit cycle',
      async function () {
        ...
        await render(hbs `
          <ClusterStorageAddForm2 @storage={{storage}} @mode={{mode}} @submit={{submit}} />
        `);
    ...
    it(
      ...
      async function () {
        ...
        await render(hbs `<ClusterStorageAddForm2 @storage={{storage}} @mode={{mode}} />`);
