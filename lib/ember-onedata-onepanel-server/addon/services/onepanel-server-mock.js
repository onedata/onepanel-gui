...
export default Ember.Service.extend({
  ...
  sessionValidator: ObjectPromiseProxy.create({
  ...
  request(api, method, ...params) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (handler && handler.success) {
        let response = {
          headers: {
            location: handler.taskId ? ('https://something/' + handler.taskId) : undefined
          }
        };
        ...
        reject(`onepanel-server-mock: mock has no method for: ${api}_${method}`);
      }
